...
const myCollection = new ArithmeticableCollection<Complex | Rational>([myComplex]);
...
describe('exercise 1 modificated tests', () => {
  ...
  it ('complex must add two complex numbers', () => {
    ...
    expect(myCollection.getArithmeticable(0)?.add(myNewComplex)).to.be.deep.equal(new Complex(3, 6));
  });

  it ('complex must substract two complex numbers', () => {
    const myNewComplex = new Complex(2, 4);
    expect(myCollection.getArithmeticable(0)?.substract(myNewComplex)).to.be.deep.equal(new Complex(-1, -2));
  });

  it ('complex must multiply two complex numbers', () => {
    const myNewComplex = new Complex(2, 4);
    expect(myCollection.getArithmeticable(0)?.multiply(myNewComplex)).to.be.deep.equal(new Complex(-6, 8));
  });

  it ('complex must divide two complex numbers', () => {
    const myNewComplex = new Complex(2, 4);
    expect(myCollection.getArithmeticable(0)?.divide(myNewComplex)).to.be.deep.equal(new Complex(0.5, 0));
  });

  it ('rational must add two rational numbers', () => {
    const myNewRational = new Rational(2, 4);
    expect(myCollection.getArithmeticable(1)?.add(myNewRational)).to.be.deep.equal(new Rational(1, 1));
  });

  it ('rational must substract two rational numbers', () => {
    const myNewRational = new Rational(2, 4);
    expect(myCollection.getArithmeticable(1)?.substract(myNewRational)).to.be.deep.equal(new Rational(0, 1));
  });

  it ('rational must multiply two rational numbers', () => {
    const myNewRational = new Rational(2, 4);
    expect(myCollection.getArithmeticable(1)?.multiply(myNewRational)).to.be.deep.equal(new Rational(1, 2));
  });

  it ('rational must divide two rational numbers', () => {
    const myNewRational = new Rational(2, 4);
    expect(myCollection.getArithmeticable(1)?.divide(myNewRational)).to.be.deep.equal(new Rational(1, 1));
  });
})